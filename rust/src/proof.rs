...
impl ProofGraph {
    ...
    /// Permutation fixpoint: detect 4 full-house rows (or cols) with all 6
    /// pairs conflicting, forming two same-parity pairs from different parallel
    /// components. A cross-pair sharing 2 position-adjacencies yields a
    /// transposition fixpoint (merge or contradiction).
    ///
    /// Returns Vec of (keep_v, remove_v, is_contradiction, partial ProofNode).
    fn find_permutation_fixpoints(&self) -> Vec<(usize, usize, bool, ProofNode)> {
        let all_houses = self.find_full_houses();
        let mut results = Vec::new();

        for is_row in [true, false] {
            // Collect full triple houses (rows or cols with 3 distinct vertices),
            // cells sorted by column (for rows) or by row (for cols).
            let mut triples: Vec<(u8, [usize; 3], [u8; 3])> = Vec::new();
            for h in &all_houses {
                if is_row && h.htype != HouseType::Row { continue; }
                if !is_row && h.htype != HouseType::Col { continue; }
            ...
            // Find parallel components via BFS
            ...
            // Collect components with ≥2 members (can form same-parity pairs)
            let big_comps: Vec<usize> = (0..components.len())
                .filter(|&c| components[c].len() >= 2)
                .collect();

            if big_comps.len() < 2 { continue; }

            // Try pairs of distinct parallel components
            for ci_idx in 0..big_comps.len() {
                for cj_idx in (ci_idx + 1)..big_comps.len() {
                    let ca = &components[big_comps[ci_idx]];
                    let cb = &components[big_comps[cj_idx]];

                    // Try all directly-linked pairs within each component
                    for ai in 0..ca.len() {
                        for aj in (ai + 1)..ca.len() {
                            let a1 = ca[ai];
                            let a2 = ca[aj];
                            if par_adj[a1] & (1 << a2) == 0 { continue; }

                            for bi in 0..cb.len() {
                                for bj in (bi + 1)..cb.len() {
                                    let b1 = cb[bi];
                                    let b2 = cb[bj];
                                    if par_adj[b1] & (1 << b2) == 0 { continue; }

                                    let quad = [a1, a2, b1, b2];
                                    let quad_mask: u32 = quad.iter()
                                        .fold(0u32, |m, &i| m | (1 << i));

                                    // All 6 pairs must conflict
                                    let all_conf = quad.iter().all(|&i|
                                        (conf_adj[i] & quad_mask) == (quad_mask & !(1u32 << i))
                                    );
                                    if !all_conf { continue; }

                                    // Try each cross-pair for 2-adjacency fixpoint
                                    for &(x, y) in &[(a1, b1), (a1, b2), (a2, b1), (a2, b2)] {
                                        let mut adj_count = 0;
                                        let mut non_adj_pos = 0usize;
                                        for k in 0..3 {
                                            if self.adj[triples[x].1[k]] & (1 << triples[y].1[k]) != 0 {
                                                adj_count += 1;
                                            } else {
                                                non_adj_pos = k;
                                            }
                                        }
                                        if adj_count != 2 { continue; }

                                        let keep_v = triples[x].1[non_adj_pos];
                                        let remove_v = triples[y].1[non_adj_pos];
                                        if keep_v == remove_v { continue; }

                                        let is_contradiction = self.adj[keep_v] & (1 << remove_v) != 0;

                                        // Build the ProofNode
                                        let house_type = if is_row { "row" } else { "col" };
                                        let vis_name = if is_row { "col" } else { "row" };
                                        ...
                                        // Parallel link descriptions for the two pairs
                                        let mut links = Vec::new();
                                        for &(p, q) in &[(a1, a2), (b1, b2)] {
                                            let (hp, _, ref oc_p) = triples[p];
                                            let (hq, _, ref oc_q) = triples[q];
                                            let via: Vec<&str> = (0..3).map(|k| {
                                                if is_row {
                                                    if oc_p[k] % 9 == oc_q[k] % 9 { "col" }
                                                    else { "box" }
                                                } else {
                                                    if oc_p[k] / 9 == oc_q[k] / 9 { "row" }
                                                    else { "box" }
                                                }
                                            }).collect();
                                            links.push(format!(
                                        ...
                                        let cross_pair = (
                                            format!("{} {}", house_type, triples[x].0 + 1),
                                            format!("{} {}", house_type, triples[y].0 + 1),
                                        );

                                        // Shared position descriptions
                                        let mut shared_desc = Vec::new();
                                        for k in 0..3 {
                                            if k == non_adj_pos { continue; }
                                            let cell_x_name = self.vertex_name(triples[x].1[k]);
                                            let cell_y_name = self.vertex_name(triples[y].1[k]);
                                            // Determine adjacency reason
                                            let oc_x = triples[x].2[k];
                                            let oc_y = triples[y].2[k];
                                            let reason = if is_row {
                                                if oc_x % 9 == oc_y % 9 {
                                                    format!("{} {}", vis_name, oc_x % 9 + 1)
                                                } else {
                                                    format!("box {}", (oc_x / 9 / 3) * 3 + (oc_x % 9 / 3) + 1)
                                                }
                                            } else {
                                                if oc_x / 9 == oc_y / 9 {
                                                    format!("{} {}", vis_name, oc_x / 9 + 1)
                                            ...
                                            shared_desc.push(format!(
                                                "{}\u{2194}{} ({})",
                                                cell_x_name, cell_y_name, reason
                                            ));
                                        }

                                        let cell_a = self.vertex_name(keep_v);
                                        let cell_b = self.vertex_name(remove_v);

                                        let node = ProofNode::PermutationFixpoint {
                                            house_type: house_type.to_string(),
                                            house_names,
                                            cells: cells_strs,
                                            links,
                                            ...
                                            cell_b: cell_b.clone(),
                                            is_contradiction,
                                            next: if is_contradiction {
                                                None
                                            } else {
                                                Some(Box::new(ProofNode::Failed))
                                            },
                                        };

                                        let (kv, rv) = (keep_v.min(remove_v), keep_v.max(remove_v));
                                        results.push((kv, rv, is_contradiction, node));
                                    }
                                }
                            }
                        }
    ...
    /// Winding chain: detect a contradiction from rows with 2 or 3 cells
    /// spanning 2 specific stacks, where winding number propagation forces
    /// 4+ rows to share the same winding but also have pairwise distinct
    /// (stack_s, stack_t) color pairs. Only 3 pairs per winding class
    /// → pigeonhole contradiction.
    ///
    /// Winding number: w(row) = (color_stack_t − color_stack_s) mod 3 ∈ {−1, +1}.
    /// Each winding class has exactly 3 color pairs.
    ///
    /// Same-winding links:
    /// - Band cycle: 3 rows in same band each with a cell in both stacks,
    ///   where the two relevant boxes each have all 3 rows represented
    ///   → derangement between boxes → same winding for all 3.
    /// - Full derangement: 2 rows (same or different band) with cells in
    ///   all 3 stacks, pairwise adjacent at each stack position via column
    ///   or box → derangement → same winding.
    fn find_gradient_chain(&self) -> Option<ProofNode> {
        // Gather (orig_cell, current_vertex) for active verts
        let mut cell_vertex: Vec<(u8, usize)> = Vec::new();
        let verts = self.active_verts();
        for &v in &verts {
            for &c in &self.labels[v] {
        ...
        // Group cells by row: row -> Vec<(orig_cell, vertex, col, stack, box)>
        let mut by_row: [Vec<(u8, usize)>; 9] = Default::default();
        for &(c, v) in &cell_vertex {
            let r = c / 9;
            by_row[r as usize].push((c, v));
        ...
        // For each pair of stacks (s1, s2) where s1 < s2, s1,s2 ∈ {0,1,2}:
        for s1 in 0u8..3 {
            for s2 in (s1 + 1)..3 {
                // s3 = the third stack
                let s3 = 3 - s1 - s2;

                // Collect "winding rows": rows that have ≥1 cell in stack s1
                // AND ≥1 cell in stack s2 (with distinct vertices)
                // Store: (row_idx, cell_s1_vertex, cell_s2_vertex, orig_s1, orig_s2,
                //         Option<cell_s3_vertex>, Option<orig_s3>)
                struct WRow {
                    row: u8,
                    v1: usize, v2: usize,      // vertices at stacks s1, s2
                    oc1: u8, oc2: u8,           // orig cells at stacks s1, s2
                    v3: Option<usize>,           // vertex at stack s3 (if any)
                    oc3: Option<u8>,             // orig cell at stack s3
                }
                ...
                // Build same-winding adjacency graph
                let mut wind_adj: Vec<u32> = vec![0; n];

                // Link type 1: band cycles
                // For each band, find rows in this stack pair.
                // If 3 rows in same band, check that box_s1 and box_s2 each
                // have one cell per row (all 3 cells in same box).
                for band in 0u8..3 {
                    let band_indices: Vec<usize> = (0..n)
                        .filter(|&i| wrows[i].row / 3 == band)
                        .collect();
                    if band_indices.len() >= 3 {
                        // For 3 rows in same band, check the box coverage
                        // Box for a cell: (row/3)*3 + col/3
                        // All stack-s1 cells in same band have same band offset
                        // and same stack → same box. Same for s2.
                        // Check that all 3 cells in each box are distinct vertices.
                        for ai in 0..band_indices.len() {
                            for bi in (ai + 1)..band_indices.len() {
                                for ci in (bi + 1)..band_indices.len() {
                                    ...
                                    // Check box_s1: all 3 in same box, all different
                                    ...
                                    // Also check distinct verts at each stack
                                    ...
                                    // All 3 rows are winding-equivalent
                ...
                // Link type 2: full derangement (2 rows, both have all 3 stacks,
                // and at each stack position the cells are adjacent)
                for i in 0..n {
                    ...
                    for j in (i + 1)..n {
                        ...
                        // Check adjacency at all 3 stacks
                ...
                // Find connected components in winding graph
                let mut visited = 0u32;
                for start in 0..n {
                    if visited & (1 << start) != 0 { continue; }
                    if wind_adj[start] == 0 { visited |= 1 << start; continue; }
                    ...
                    visited |= 1 << start;
                    while let Some(u) = queue.pop_front() {
                        let mut nbrs = wind_adj[u] & !visited;
                        while nbrs != 0 {
                            let v = nbrs.trailing_zeros() as usize;
                            nbrs &= nbrs - 1;
                            visited |= 1 << v;
                    ...
                    // Two rows i,j have distinct (s1,s2) pairs if:
                    // (a) their s1 cells are adjacent, OR
                    // (b) their s2 cells are adjacent, OR
                    // (c) both are full 3-stack rows with adjacent s3 cells
                    //     → within same winding class, different s3 → different pair
                    ...
                    // Try 4-subsets for pigeonhole contradiction
                    let nm = members.len();
                    for a in 0..nm {
                        for b in (a + 1)..nm {
                            for c in (b + 1)..nm {
                                for d in (c + 1)..nm {
                                    ...
                                    // All members are already in the same connected component,
                                    // so they share the same gradient δ. No need to check
                                    // internal connectivity of this 4-subset.

                                    // All 6 pairs must be distinct
                                    let all_distinct = sub.iter().all(|&i|
                                        (dist_adj[i] & sub_mask) == (sub_mask & !(1u32 << i))
                                    );
                                    if !all_distinct { continue; }

                                    // Found a contradiction! Build the proof node.

                                    // BFS from lowest-degree node for natural chain ordering
                                    let mut ordered: Vec<usize> = Vec::new();
                                    {
                                        let start = members.iter()
                                            .min_by_key(|&&i| (wind_adj[i] & comp).count_ones())
                                            .copied().unwrap();
                                        ...
                                        while let Some(u) = bfs_queue.pop_front() {
                                            ordered.push(u);
                                            let mut nbrs = wind_adj[u] & comp & !bfs_visited;
                                            while nbrs != 0 {
                                                let v = nbrs.trailing_zeros() as usize;
                                                nbrs &= nbrs - 1;
                                                bfs_visited |= 1 << v;
                                    ...
                                    // Helper: describe house link between two cells
                                    ...
                                    // Build gradient link descriptions
                                    let mut gradient_links: Vec<String> = Vec::new();

                                    // Describe derangement links (between BFS-adjacent pairs)
                                    for &i in &ordered {
                                        if wrows[i].v3.is_none() { continue; }
                                        for &j in &ordered {
                                            if j <= i { continue; }
                                        ...
                                        for sj in (si + 1)..4 {
                                            ...
                                            if adj_s1 {
                                                distinctness.push(format!(
                                                    ...
                                                    wrows[i].row + 1, wrows[j].row + 1,
                                                    house_desc(wrows[i].oc1, wrows[j].oc1),
                                                ));
                                            } else if adj_s2 {
                                                distinctness.push(format!(
                                                    "r{}\u{2194}r{}: {} \u{2192} pairs differ.",
                                                    wrows[i].row + 1, wrows[j].row + 1,
                                                    house_desc(wrows[i].oc2, wrows[j].oc2),
                                                ));
                                            } else if adj_s3 {
                                    ...
                                    return Some(ProofNode::GradientChain {
                                        chain_rows,
                                        gradient_links,
                                        contra_rows,
                                        distinctness,
...
pub enum ProofNode {
    ...
    /// Permutation fixpoint: 4 full-house rows (or columns) with all pairs
    /// conflicting, forming two same-parity pairs from different parallel
    /// components. Distinctness forces opposite inter-pair parity; a cross-pair
    /// sharing 2 position adjacencies yields a transposition fixpoint (merge
    /// or contradiction if the fixpoint cells are adjacent).
    PermutationFixpoint {
        house_type: String,
        house_names: Vec<String>,       // all 4 house names
        cells: Vec<[String; 3]>,        // cells per house sorted by position
        links: Vec<String>,             // parallel link descriptions for the two pairs
        cross_pair: (String, String),   // the two houses yielding the fixpoint
        shared_desc: Vec<String>,       // shared position adjacency descriptions
        cell_a: String,                 // fixpoint cell from first cross house
        cell_b: String,                 // fixpoint cell from second cross house
        is_contradiction: bool,         // true if cell_a, cell_b are adjacent
        next: Option<Box<ProofNode>>,   // None if contradiction, Some if merge
    },
    /// Gradient chain: 4+ rows sharing a conserved color-difference δ
    /// (via circular-ladder-3 and full-derangement links). 4 rows with
    /// pairwise distinct color pairs but only 3 per δ class → pigeonhole.
    GradientChain {
        chain_rows: Vec<(String, Vec<String>)>,  // (row_name, cell names) in chain traversal order
        gradient_links: Vec<String>,             // how same δ is established
...
impl ProofNode {
    ...
    pub fn depth(&self) -> usize {
        match self {
            ...
            | ProofNode::ParityChain { .. } | ProofNode::GradientChain { .. }
    ...
    pub fn odd_wheel_count(&self) -> usize {
        match self {
            ...
            | ProofNode::ParityTransport { .. } | ProofNode::ParityChain { .. } | ProofNode::GradientChain { .. }
            | ProofNode::HouseColoringContradiction { .. }
            | ProofNode::PigeonholeXwing { .. }
            | ProofNode::Failed => 0,
    ...
    pub fn technique_signature(&self) -> u32 {
        match self {
            ...
            | ProofNode::GradientChain { .. } | ProofNode::HouseColoringContradiction { .. } => TECH_PARITY,
            ProofNode::Failed => 0,
...
fn find_best_proof(
    ...
    // Terminal: winding chain (band parity pigeonhole with partial rows)
    if disabled & TECH_PARITY == 0 {
        if let Some(wc) = graph.find_gradient_chain() {
            return wc;
        }
    }
    ...
    // Try permutation fixpoint deductions (2 same-parity pairs, opposite parity → fixpoint merge)
    if disabled & TECH_PARITY == 0 {
    let fps = graph.find_permutation_fixpoints();
    for (keep, remove, is_contradiction, partial_node) in fps {
        if is_contradiction {
            // Terminal — size 1
            if 1 < best_size {
                best_size = 1;
                best = Some(partial_node);
            }
            continue;
        }
        let mut g = graph.clone();
        ...
        if next.is_complete() {
            ...
            if total < best_size {
                best_size = total;
                if let ProofNode::PermutationFixpoint {
                    house_type, house_names, cells, links,
                    cross_pair, shared_desc, cell_a, cell_b,
                    is_contradiction: ic, ..
                } = partial_node {
                    best = Some(ProofNode::PermutationFixpoint {
                        house_type,
                        house_names,
                        cells,
                        links,
...
fn find_greedy_proof(
    ...
    // Priority 3.6: permutation fixpoint (after parity chain deduction, before ladder)
    let fps = graph.find_permutation_fixpoints();
    if let Some((keep, remove, is_contradiction, partial_node)) = fps.into_iter().next() {
        if is_contradiction {
            return partial_node;
        }
        let mut g = graph.clone();
        ...
        if let ProofNode::PermutationFixpoint {
            ...
            return ProofNode::PermutationFixpoint {
                house_type,
                house_names,
                cells,
                links,
    ...
    match node {
        ...
        ProofNode::GradientChain { chain_rows, gradient_links, contra_rows, distinctness } => {
            *step += 1;
            ...
            s += &format!(
                "{}    \u{2192} {} have same \u{03b4}. 4 rows; only 3 color pairs per \u{03b4} value.\n",
        ...
        ProofNode::PermutationFixpoint {
            house_type, house_names, cells, links,
            cross_pair, shared_desc, cell_a, cell_b,
            is_contradiction, next
        } => {
            *step += 1;
            let vis_name = if house_type == "row" { "column" } else { "row" };
            let mut s = format!(
                "{}{}.  Permutation fixpoint:\n",
                pad, step,
            );
            for i in 0..house_names.len() {
                s += &format!(
